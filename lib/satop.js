
const contrib = require('blessed-contrib')
    , {donut, lcd, line, sparkline, table} = contrib
    , blessed = require('blessed')
    , {box, text} = blessed;

const {block} = require('./widget') //customized Blessed widgets
    , bitcoin = require('./bitcoin')
    , system = require('./system');


module.exports = {
  init: async opts => {

    let cpu, net, mem, disk, procs, temp
      , nodeinfo, fees, mempool, blocks, peers, tor;

    const screen = blessed.screen({
      title: 'SatoshiTop',
      smartCSR: true,
      //fastCSR: true
    });

    const grid = new contrib.grid({
      screen,
      rows: 12,
      cols: 12,
    });

    //row, col, rowSpan, colSpan, obj, opts
    cpu = grid.set(0, 0, 3, 4, line, {
      label: 'Cpu',
      showNthLabel: 5,
      maxY: 100,
      showLegend: false,
    });

    net = grid.set(3, 0, 3, 4, sparkline, {
      label: 'Network',
      tags: true,
      style: {
        fg: 'blue',
      },
    });

    mem = grid.set(0, 4, 4, 3, donut, {
      label: 'Ram',
      radius: 10,
      arcWidth: 6,
      yPadding: 0,
      remainColor: 'black',
    });

    disk = grid.set(0, 7, 4, 5, donut, {
      label: 'Disks',
      radius: 10,
      arcWidth: 6,
      yPadding: 0,
      remainColor: 'black',
      showLegend: true,
    });

    procs = grid.set(4, 7, 4, 5, table, {
      label: 'Processes',
      keys: true,
      interactive: false,
      columnSpacing: 2,
      columnWidth: [4, 4, 6, 20],
    });

    temp = grid.set(4, 4, 2, 3, lcd, {
      label: 'Temperature',
      elements: 3,
      display: '00.0C',
      elementPadding: 0,
      color: 'red'
    });

    nodeinfo = grid.set(6, 0, 2, 3, box, {
      label: 'Bitcoin Node',
      tags: true,
      style: {
        fg: 'yellow'
      }
    });

    fees = grid.set(6, 3, 2, 4, block, {
      label: 'Fees',
      tags: true,
      style: {
        fg: 'yellow'
      },
      barBgColor: 'yellow',
      barFgColor: 'black',
      labelColor: 'yellow',
      showText: true,
      barWidth: 7,
      barSpacing: 3,
      xOffset: 0,
      maxHeight: 100
    });

    mempool = grid.set(8, 0, 3, 3, box, {
      label: 'Mempool',
      tags: true,
      style: {
        fg: 'yellow'
      }
    });

    blocks = grid.set(8, 3, 3, 4, block, {
      label: 'Blocks',
      tags: true,
      style: {
        fg: 'yellow'
      },
      barFgColor: 'white',
      labelColor: 'blue',
      showText: true,
      barWidth: 7,
      barSpacing: 3,
      xOffset: 0,
      maxHeight: 2097152,
    });

    peers = grid.set(8, 7, 3, 5, table, {
      label: 'Peers',
      keys: true,
      interactive: false,
      columnSpacing: 0,
      columnWidth: [18, 30],
    });

    tor = grid.set(11, 0, 1, 12, box, {
      label: 'Tor Network',
      tags: true,
      style: {
        fg: 'blue'
      }
    });

    screen.render();

    screen.on('resize', e => {
      //system
      if(cpu) cpu.emit('attach');
      if(net) net.emit('attach');
      if(mem) mem.emit('attach');
      if(temp) temp.emit('attach');
      if(disk) disk.emit('attach');
      if(procs) procs.emit('attach');

      //bitcoin
      if(tor) tor.emit('attach');
      if(nodeinfo) nodeinfo.emit('attach');
      if(fees) fees.emit('attach');
      if(blocks) blocks.emit('attach');
      if(peers) peers.emit('attach');
      if(mempool) mempool.emit('attach');
    });

    screen.key(['escape', 'q', 'C-c'], (ch, key) => {
      return process.exit(0);
    });

    //system
    new system.cpu(cpu, opts);
    new system.net(net, opts);
    new system.mem(mem, opts);
    new system.disk(disk, opts);
    new system.proc(procs, opts);
    new system.temp(temp, opts);

    //bitcoin
    await bitcoin.rpc.connect(opts);
    if(nodeinfo) new bitcoin.nodeinfo(nodeinfo, opts);
    if(fees) new bitcoin.fees(fees, opts);
    if(blocks) new bitcoin.blocks(blocks, opts);
    if(mempool) new bitcoin.mempool(mempool, opts);
    if(peers) new bitcoin.peers(peers, opts);
    if(tor) new bitcoin.tor(tor, opts);
  }
};
