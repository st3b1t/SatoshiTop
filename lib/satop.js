...
const {block, line, table, gauge} = require('./widget') //customized Blessed widgets
...
/*    if(mempool)
    mempool = grid.set(8, 0, 3, 4, gauge, {
      label: 'Mempool',
      tags: true,
      style: {
        fg: 'yellow'
  ...
  mempool = grid.set(8, 0, 3, 4, contrib.gauge,
      {
        gaugeHeight: 3,
        stack: [
          {percent: 20, stroke: 'white'},
          {percent: 30, stroke: 'yellow'},
          {percent: 40, stroke: 'red'}
        ]
      }
    )
